...
@section('profile-content')
    ...
    <div class="row justify-content-center">
        ...
        <div class="col-md-8">
            ...
            @else
                <h3 class="mb-3">Conversation with {{ $conversation -> otherUser() -> username }} <img class="image rounded-circle" src="{{asset('/storage/images/'.$conversation->otherUser()->image)}}" alt="" style="width: 30px; height: 30px; padding: 0px; margin: 0px; "></h3>
                {{-- User is not a Stub --}}

                @if($conversation -> otherUser() -> exists)
                <form action="{{ route('profile.messages.message.new', $conversation) }}" method="POST">
                    <textarea class="form-control" rows="5" name="message"
                              placeholder="Type here to send..."></textarea>
                    <button type="submit" class="btn btn-primary ms-auto mt-2 d-block">Send message</button>
                    {{ csrf_field() }}
                </form>
                @endif
