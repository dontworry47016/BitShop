...
trait Purchasable {
    ...
    public function sent()
    {
        // checking for vendor
        if(!$this -> isVendor())
            throw new RequestException('Bruh wtf you gotta be the seller to mark this sale as sent.');
    ...
    public function delivered()
    {
        ...
        // state must be 'sent' to be delivered
        throw_unless($this->state=='sent', new RequestException('This purchase has not been marked shipped!'));
        ...
        try{
            ...
            // state now must be delivered
            throw_unless($this->state=='delivered', new \Exception('This purchase is already marked delivered!'));
            $this -> getPayment() -> delivered();
    ...
    public function complete()
    {
        ...
        throw_if($this->type!='fe', new RequestException('This purchase is not FE!'));
        // checking if it is not in purchased state
        throw_if(!$this->isCompletable(), new RequestException('The purchase you selected is not in sent state or product is not autodelivery!'));
    ...
    public function makeDispute($message)
    {
        if(!$this->canMakeDispute())
            throw new RequestException('You haven\'t paid for this order, so you can\'t dispute it.');
    ...
    public function resolveDispute(string $winnerId)
    {
        ...
        throw_if($this->isDisputed() && $this->dispute->isResolved(), new RequestException("This dispute is already resolved?"));
        ...
        if(!$this -> isBuyer($winner) && !$this -> isVendor($winner))
            throw new RequestException('User is not vendor or buyer, this is was reported to the owner.');
